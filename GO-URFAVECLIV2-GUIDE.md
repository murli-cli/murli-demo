# Murli Integration Guide: Go with urfave/cli v2

This guide explains how to systematically integrate the **Murli CLI middleware** (`github.com/murli-cli/murli-go`) into a Go application built with the popular **urfave/cli/v2** CLI framework.

---

## 🎁 What You Get for Free

By simply replacing urfave/cli's standard execution call (`app.Run(os.Args)`) with Murli's adapter entry point (`murliCli.Run(app, os.Args)`), your CLI tool automatically inherits the following powerful features:
...
## ⚠️ Key Library Nuance: Strict Flag Parsing Order
**urfave/cli v2** enforces strict positional argument ordering during parsing. Any options or flags (like `--agent` or `--force`) specified *after* a positional argument (like the task title) are treated as literal arguments rather than parsed options.

To ensure your commands are processed correctly by AI agents, **always place the flags before the positional arguments**:

*   ❌ **Incorrect:** `murli-work task create "Verify Urfave V2" --agent --force`
*   icon:check **Correct:** `murli-work task create --agent --force "Verify Urfave V2"`
...
Add the Murli core and urfave/cli v2 adapter to your imports in `main.go`:
...
Update your subcommand `Action` blocks to capture and format outputs dynamically:
...
### 1. Human TTY Help Command (`./bin/murli-work-go-urfave --help`)
```text
NAME:
...
### 2. Mutating Confirmation Prompt Safeguard (`./bin/murli-work-go-urfave init --agent`)
...
### 3. Bypassing Safeguard with Force (`./bin/murli-work-go-urfave init --agent --force`)
```json
{
  "result": {
...
### 4. Creating a Task in Agent Mode (Proper Strict Flag Ordering) (`./bin/murli-work-go-urfave task create --agent --force --priority high "Verify Urfave V2"`)
```json
{
  "result": {
    "id": 6,
    "title": "Verify Urfave V2"
  },
  "schema_version": "1.0",
  "status": "ok"
}
...
### 5. Listing Tasks in Agent Mode (`./bin/murli-work-go-urfave task list --agent`)
```json
{
  "result": [
    {
...
### 6. Sprint Report in Agent Mode (`./bin/murli-work-go-urfave report --agent`)
```json
{
  "result": {
    "completed_tasks": 2,
